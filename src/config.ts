...
export function parseConfig(raw: unknown): Config {
  ...
  if (!validPath(value.database))
    throw new Error("config.database must be a nonempty path");
  if (
    value.welcomeOnJoin !== undefined &&
    typeof value.welcomeOnJoin !== "boolean"
  )
  ...
  for (let index = 0; index < bots.length; index++) {
    ...
    for (
      let channelIndex = 0;
      channelIndex < bot.channels.length;
      channelIndex++
    ) {
...
function validateBot(raw: unknown, index: number): BotConfig {
  ...
  if (!nickToken(value.nick)) throw new Error(`${path}.nick is invalid`);
  if (!serverToken(value.server)) throw new Error(`${path}.server is invalid`);
...
function validateChannels(raw: unknown, botPath: string): string[] {
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    if (keys.has(key)) throw new Error(`${path}[${index}] is duplicated`);
    keys.add(key);
...
function validateAccountAuth(
  ...
  if (value.method !== "sasl") throw new Error(`${path}.method is unsupported`);
  if (!token(value.username)) throw new Error(`${path}.username is invalid`);
...
function validateServiceAuth(
  ...
  if (!token(value.target)) throw new Error(`${path}.target is invalid`);
...
function optionalPassword(value: unknown, path: string): string | undefined {
  ...
  if (!validPassword(value)) throw new Error(`${path} is invalid`);
  return value;
}
