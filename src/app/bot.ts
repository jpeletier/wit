...
export class WitBot {
  ...
  handle(event: IrcEvent): void {
    try {
      ...
      if (channel !== undefined && this.#sessions.has(this.#key(channel))) {
        ...
        this.irc.notice(event.user.nick, `No se pudo ejecutar el comando: ${message(error)}`);
      }
    }
  }
    ...
    switch (event.type) {
      ...
      case "part":
        if (event.self) {
          ...
          this.#end(event.channel, "La partida terminó porque el bot salió del canal");
        }
        break;
      case "kick":
        if (event.self) {
          ...
          this.#end(event.channel, "La partida terminó porque el bot fue expulsado del canal");
        }
        break;
      case "disconnected":
    ...
    for (const session of [...this.#sessions.values()]) {
      try {
        ...
        if (session.type === "trivia") {
          this.#triviaTick(session);
        } else {
          this.#cylTick(session);
        }
      } catch (error) {
  ...
  #private(user: IrcUser, text: string): void {
    ...
    if (command === "HELP") {
      for (const line of HELP) {
        this.irc.notice(user.nick, line);
      }
      return;
    }
    if (command === "DATE") {
    ...
    if (parts[2]?.toUpperCase() === "STOP") {
      ...
      if (!this.irc.isOperator(channel, user.nick)) {
        this.irc.notice(user.nick, `Sólo un operador (@) de ${channel} puede detener la partida.`);
        return;
      }
      this.irc.say(
        channel,
        mirc.color(`--- Partida detenida por ${mirc.underline(user.nick)} ---`, 4)
      );
      this.#end(channel);
      return;
    }
    ...
    let count = parts[2] === undefined || !Number.isFinite(parsed) ? 20 : Math.trunc(parsed);
    if (type === "trivia" && count < 5) {
      count = 5;
    }
    if (type === "cyl" && count <= 0) {
    ...
    if (type === "trivia") {
      this.#startTrivia(channel, user, count);
    } else {
  ...
  #startTrivia(channel: string, user: IrcUser, count: number): void {
    const context = this.games.prepareTournament(this.options.networkId, channel, 1);
    ...
    const gameId = this.games.createGame(context.tournamentId, count, "Trivial");
  ...
  #startCyl(channel: string, user: IrcUser, count: number): void {
    const context = this.games.prepareTournament(this.options.networkId, channel, 2);
    const gameId = this.games.createGame(context.tournamentId, count, "Cifras y Letras");
  ...
  #announceContext(channel: string, context: TournamentContext, game: string): void {
    if (context.firstGameInChannel) {
      this.irc.say(channel, "Es la primera partida que se juega en este canal.");
    }
    if (context.newTournament) {
      this.irc.say(channel, mirc.bold(`¡Bienvenidos a la ${context.leagueId}ª liga de ${game}!`));
    }
  }
  ...
  #channel(channel: string, user: IrcUser, text: string): void {
    ...
    if (session?.type === "cyl" && session.round !== undefined) {
      ...
      if (accepted && session.round.winner()?.distance === 0) {
        session.tick = 60;
      }
      return;
    }
  ...
  #showTrivia(session: TriviaSession, event: TriviaEvent): void {
    ...
    if (event.type === "question") {
      ...
      this.irc.say(channel, `${mirc.color(event.question.text, 2)} (${event.words} pal.)`);
  ...
  #cylTick(session: CylSession): void {
    if (session.tick === 5) {
      ...
      if (session.challenge % 4 === 0) {
        ...
        this.irc.say(session.channel, `${cylHeader()} Desafío #${session.challenge} a CIFRAS`);
        this.irc.say(
          session.channel,
        ...
        session.round = new LetterRound(letters, (key) => this.dictionary.lookup(key));
        this.irc.say(session.channel, `${cylHeader()} Desafío #${session.challenge} a LETRAS`);
        this.irc.say(
          session.channel,
    ...
    if (session.tick === 40 && session.round !== undefined) {
      this.irc.say(session.channel, `20 segundos ... ${this.#roundReminder(session.round)}`);
    }
    if (session.tick < 60 || session.round === undefined) {
      return;
    }
    const changes: Array<{
      player: PlayerState;
    ...
    if (session.round instanceof LetterRound) {
      ...
      if (winners.length === 0) {
        this.irc.say(session.channel, "Se acabó el tiempo. Nadie supo construir una palabra.");
      } else {
        this.irc.say(session.channel, this.#letterResult(session.round, winners));
      ...
      if (winner === undefined) {
        this.irc.say(session.channel, "Se acabó el tiempo. Nadie consiguió la cifra.");
      } else {
        this.irc.say(
          session.channel,
    ...
    for (const change of changes) {
      change.player.total += change.points;
    }
    this.#standings(
      session,
    ...
    session.tick = 0;
    if (session.challenge >= session.count) {
      this.#complete(session, "C&L");
    }
  }
  ...
  #letterResult(round: LetterRound, winners: readonly LetterWinner[]): string {
    ...
    if (winners.length > 1) {
      text += `. Otras palabras: ${winners
        .slice(1)
        .map((winner) => `${winner.word} (${winner.nick} ${mirc.color(`+${winner.score}`, 4)}) `)
        .join("")}`;
    }
    return text;
  }
  ...
  #standings(session: Session, deltas = new Map<string, number>()): void {
    ...
    const prefix = session.type === "trivia" ? `${mirc.bold("Puntuación:")} ` : "Puntuaciones: ";
    const body = players
      .map((player, index) =>
        session.type === "trivia"
  ...
  #complete(session: Session, gameName: "trivial" | "C&L"): void {
    this.irc.say(session.channel, `Acabó la partida de ${gameName}. Puntuaciones: `);
    ...
    ordered.forEach((player, index) => {
      ...
      this.irc.say(
        session.channel,
        mirc.color(`${mirc.bold(`${index + 1}º`)} ${player.nick} ${player.total}${general}`, 1, 15)
      );
    });
  ...
  #rekeySessions(): void {
    ...
    for (const [key, sessions] of grouped) {
      ...
      for (const session of sessions) {
        this.#finishSession(session, "La partida terminó por un conflicto de CASEMAPPING");
      }
    }
  }
  ...
  #recordJoin(channel: string): void {
    ...
    if (pending?.invitedBy !== undefined) {
      this.irc.say(channel, `Hola, ${pending.invitedBy.nick} me ha invitado aquí.`);
      this.irc.say(
        channel,
  ...
  #recordMessage(channel: string, user: IrcUser): void {
    if (this.#sameNick(user.nick, this.irc.nick)) {
      return;
    }
  ...
  #invite(channel: string, user: IrcUser): void {
    ...
    const occupied = new Set([
      ...
      ...[...this.#replacementJoins.values()].map((replacement) => this.#key(replacement.channel)),
    ]).size;
    if (occupied >= this.options.channelLifecycle.maxChannels) {
      const now = this.clock.now().getTime();
      const minimumIdle = this.options.channelLifecycle.inviteEvictionIdleMinutes * MINUTE_MS;
  ...
  #maintainChannels(): void {
    const now = this.clock.now().getTime();
    const messageIdle = this.options.channelLifecycle.messageIdleMinutes * MINUTE_MS;
    ...
    for (const activity of this.#channelActivity.values()) {
      ...
      const noRecentGame = activity.receivedMessage && now - gameReference >= gameIdle;
      if (!silent && !noRecentGame) {
        continue;
      }
  ...
  #rekeyChannels(): void {
    ...
    for (const activity of activities) {
      ...
      const existing = this.#channelActivity.get(key);
      if (existing === undefined) {
        this.#channelActivity.set(key, activity);
      } else {
        existing.joinedAt = Math.min(existing.joinedAt, activity.joinedAt);
        existing.lastMessageAt = Math.max(existing.lastMessageAt, activity.lastMessageAt);
...
function latestTime(left: number | undefined, right: number | undefined): number | undefined {
  if (left === undefined) {
    return right;
  }
  if (right === undefined) {
